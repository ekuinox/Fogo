...
private:
	...
	{
		constexpr D3D12_DESCRIPTOR_RANGE range[1] = { D3D12_DESCRIPTOR_RANGE {
		...
		constexpr auto samplerDesc = D3D12_STATIC_SAMPLER_DESC {
...
public:
	...
	PlainSquare(ID3D12Device * device, const SquareDetail & squareDetail)
		: vertexBuffer({}), constantBuffer({})
	{
		...
		buffer[0].position = { squareDetail.left(), squareDetail.top(), 0.0f };
		buffer[1].position = { squareDetail.right(), squareDetail.top(), 0.0f };
		buffer[2].position = { squareDetail.left(), squareDetail.buttom(), 0.0f };
		buffer[3].position = { squareDetail.right(), squareDetail.buttom(), 0.0f };

		for (auto i = 0; i < SquareDetail::vertexesCount; ++i)
		{
	...
	void render(ID3D12GraphicsCommandList * commandList) const
	{
		...
		*buffer = Mat;
		...
		const auto & vertexView = D3D12_VERTEX_BUFFER_VIEW {
			...
			sizeof(TexturedVertex3D) * SquareDetail::vertexesCount,
		...
		//描画
		commandList->DrawInstanced(SquareDetail::vertexesCount, 1, 0, 0);
	}
};